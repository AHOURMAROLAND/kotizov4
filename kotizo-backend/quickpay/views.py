...
from core.utils import calculer_frais_kotizo, detecter_operateur_togo
...
class QuickPayListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if limite and user.cotisations_creees_aujourd_hui >= limite:
            return Response(
                {'error': f'Limite de {limite} operations par jour atteinte'},
        ...
        return Response(
            QuickPaySerializer(quickpay).data,
...
class QuickPayDetailView(APIView):
    ...
    def get(self, request, code):
        ...
        if quickpay.statut == 'expire':
            return Response(
                {'error': 'Ce lien Quick Pay a expire', 'code': 'expire'},
...
class InitierPaiementQuickPayView(APIView):
    ...
    def post(self, request, code):
        ...
        if quickpay.date_expiration <= timezone.now():
            ...
            quickpay.save(update_fields=['statut'])
            return Response(
                {'error': 'Ce lien Quick Pay a expire'},
                status=status.HTTP_410_GONE
            )
...
class QuickPayRecusView(APIView):
    ...
    def get(self, request):
        quickpays = QuickPay.objects.filter(
            payeur=request.user,
            statut='paye'
        )
        return Response(QuickPaySerializer(quickpays, many=True).data)