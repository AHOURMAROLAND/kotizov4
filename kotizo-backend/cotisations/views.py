...
from .utils import (
    generer_slug, get_cotisations_actives_cache,
    set_cotisations_actives_cache, invalider_cache_cotisations,
)
...
class CotisationListCreateView(APIView):
    ...
    def get(self, request):
        cached = get_cotisations_actives_cache(str(request.user.id))
        if cached:
        ...
        set_cotisations_actives_cache(str(request.user.id), serializer.data)
    ...
    def post(self, request):
        ...
        limite = user.get_limite_cotisations()
        if limite and user.cotisations_creees_aujourd_hui >= limite:
            return Response(
                {'error': f'Limite de {limite} cotisations par jour atteinte'},
        ...
        user.save(update_fields=['cotisations_creees_aujourd_hui'])
...
class RejoindreView(APIView):
    ...
    def post(self, request, slug):
        ...
        if cotisation.est_privee and cotisation.createur != request.user:
            return Response(
                {'error': 'Cette cotisation est privee'},
                status=status.HTTP_403_FORBIDDEN
            )
...
class SignalerView(APIView):
    ...
    def post(self, request, slug):
        ...
        if nb_signalements >= 3:
            ...
            cotisation.save(update_fields=['statut'])
            logger.fraude(f'Cotisation {slug} suspendue - 3 signalements')
...
class CotisationPubliqueView(APIView):
    ...
    def get(self, request, slug):
        ...
        if cotisation.statut == 'annulee':
            return Response(
                {'error': 'Ce lien a expire ou la cotisation a ete supprimee', 'code': 'lien_expire'},
